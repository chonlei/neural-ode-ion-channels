...
parser.add_argument('--info', type=str, default='s00')
...
makedirs('r1-tune/%s' % args.info)


suffix = '-2'

def save_ckp(state, isbest=False, checkpoint_dir='r1-tune/%s' % args.info):
...
ckp_path = 'r1-tune/%s/checkpoint%s.pt' % (args.info, suffix)
...
info_id = args.info
info = importlib.import_module(args.info)
...
if args.pred:
    ...
    #func.load_state_dict(torch.load('r1-tune/%s/model-state-dict.pt' % args.info))
    best_checkpoint = torch.load('r1-tune/%s/best-model-checkpoint.pt' % args.info)
    ...
    ax.set_ylabel('a')
    ax.set_zlabel('da/dt')
    plt.savefig('r1-tune/{:s}/rates3d-{:03d}'.format(args.info, ii), dpi=200)
    #plt.show()
    plt.close()
    ...
    with torch.no_grad():
        ...
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('r1-tune/{:s}/{:03d}-aps'.format(args.info, ii), dpi=200)
        #plt.show()
        plt.close(fig1)
        ...
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('r1-tune/{:s}/{:03d}-sinewave'.format(args.info, ii), dpi=200)
        #plt.show()
        plt.close(fig1)
        ...
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('r1-tune/{:s}/{:03d}-pr3'.format(args.info, ii), dpi=200)
        # do another one with zooms
        ax1.set_xlim(5000, 7000)
        ax1.set_ylim(-2, 1.7)
        fig1.tight_layout()
        fig1.savefig('r1-tune/{:s}/{:03d}-pr3-z'.format(args.info, ii), dpi=200)
        #plt.show()
        plt.close(fig1)
        ...
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('r1-tune/{:s}/{:03d}-pr4'.format(args.info, ii), dpi=200)
        # do another one with zooms
        ax1.set_xlim(1175, 1475)
        ax1.set_ylim(-2.5, 7)
        fig1.tight_layout()
        fig1.savefig('r1-tune/{:s}/{:03d}-pr4-z'.format(args.info, ii), dpi=200)
        #plt.show()
        plt.close(fig1)
...
if args.cached:
    ...
    if args.smoothi:
        ...
        didt_batches3 = []
        for j, (i, v) in enumerate(zip(i_batches3, v_batches3)):
            ii = i.cpu().numpy().reshape(-1)
            pt = time3
            pv = voltage3
            cc = change_pt3
    ...
    for i, (v, a, dadt) in enumerate(zip(v_batches3, a_batches3, dadt_batches3)):
        ...
        if args.smoothi:
            d2adt2_batches3[i] = d2adt2_batches3[i][mask3,...][skip::sparse]
    # Not using sine wave
...
if __name__ == '__main__':
    ...
    # Save model
    torch.save(func.state_dict(), 'r1-tune/%s/model-state-dict.pt' % args.info)
    torch.save(func, 'r1-tune/%s/model-entire.pt' % args.info)
    # To load model:
    ...
    #
    # Or:
    # func = torch.load('r1-tune/%s/model-entire.pt' % args.info)
    # func.eval()
    ...
    with torch.no_grad():
        ...
        ax1.set_xlim(timep1.min(), timep1.max())
        fig1.tight_layout()
        fig1.savefig('r1-tune/{:s}/{:03d}'.format(args.info, ii), dpi=200)
        plt.close(fig1)
