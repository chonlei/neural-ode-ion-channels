...
parser = argparse.ArgumentParser('IKr discrepancy fit with NN-d.')
...
if args.pred:
    ...
    func.load_state_dict(torch.load('d2/model-state-dict.pt'))
    #func = torch.load('d2/model-entire.pt').to(device)
    func.eval()
    ...
    with torch.no_grad():
        ...
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}-ap2hz'.format(ii), dpi=200)
        plt.close(fig1)
        ...
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}-aps'.format(ii), dpi=200)
        plt.close(fig1)
        ...
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}-sinewave'.format(ii), dpi=200)
        plt.close(fig1)
        ...
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}-staircase'.format(ii), dpi=200)
        plt.close(fig1)
        ...
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}-act'.format(ii), dpi=200)
        plt.close(fig1)
        ...
        ax1.set_xlim(t.cpu().min(), t.cpu().max())
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}-deact'.format(ii), dpi=200)
        plt.close(fig1)
        ...
        ax1.set_xlim(t.cpu().min(), t.cpu().max())
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}-atau'.format(ii), dpi=200)
        plt.close(fig1)
...
###
if args.cached:
    v_batches = torch.load('d2/v.pt')
    a_batches = torch.load('d2/a.pt')
    dadt_batches = torch.load('d2/dadt.pt')
else:
    skip = 5  # not accurate for the first few time points for estimating derivatives
    sparse = 21  # use less data points
    ...
    for j, (i, protocol) in enumerate(zip(i_batches1, protocol_batches1)):
        ...
        didto = []
        for t_f in t_split:
            idx = np.where((t1.cpu().numpy() >= t_i) & (t1.cpu().numpy() < t_f))[0]
            tfit = t1.cpu().numpy()[idx]
            ifit = smooth(ii[idx], 151)[75:-75]  # smoothing with 151/10ms
            spl = UnivariateSpline(tfit, ifit, k=3)
            spl.set_smoothing_factor(1)  # s is the smoothing factor; if 0, spline will interpolate through all data points
            io = np.append(io, spl(tfit))
            didto = np.append(didto, spl.derivative()(tfit))
            t_i = t_f
        if False:
    ...
    for j, (i, protocol) in enumerate(zip(i_batches2, protocol_batches2)):
        ...
        didto = []
        for t_f in t_split:
            idx = np.where((t2.cpu().numpy() >= t_i) & (t2.cpu().numpy() < t_f))[0]
    ...
    # Cache it
    torch.save(v_batches, 'd2/v.pt')
    torch.save(a_batches, 'd2/a.pt')
    torch.save(dadt_batches, 'd2/dadt.pt')
...
if __name__ == '__main__':
    ...
    k1 = true_model.p1 * torch.exp(true_model.p2 * X1) * 0.3  # NOTE: shift the pretrain
    k2 = true_model.p3 * torch.exp(-true_model.p4 * X1) * 3.
    ...
    # Save model
    torch.save(func.state_dict(), 'd2/model-state-dict.pt')
    torch.save(func, 'd2/model-entire.pt')
    # To load model:
    ...
    #
    # Or:
    # func = torch.load('d2/model-entire.pt')
    # func.eval()
    ...
    with torch.no_grad():
        ...
        ax1.set_xlim(prediction_t.cpu().min(), prediction_t.cpu().max())
        fig1.tight_layout()
        fig1.savefig('d2/{:03d}'.format(ii), dpi=200)
        plt.close(fig1)
        ...
        ax.set_ylabel('a')
        ax.set_zlabel('da/dt')
        plt.savefig('d2/rates3d-{:03d}'.format(ii), dpi=200)
        #plt.show()
        plt.close()
