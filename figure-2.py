...
if args.cached:
    current1 = torch.load('figure-2/yc-pr3.pt')
    pred_y_o_pr3 = torch.load('figure-2/yo-pr3.pt')
    pred_y_1_pr3 = torch.load('figure-2/y1-pr3.pt')
    pred_y_2_pr3 = torch.load('figure-2/y2-pr3.pt')
    currentp3 = torch.load('figure-2/yc-pr5.pt')
    pred_y_o_pr5 = torch.load('figure-2/yo-pr5.pt')
    pred_y_1_pr5 = torch.load('figure-2/y1-pr5.pt')
    pred_y_2_pr5 = torch.load('figure-2/y2-pr5.pt')
else:
    with torch.no_grad():
        ###
        ### Training protocols
        ...
        for i in range(7):
            ...
            ax2.legend()
            fig2.tight_layout()
            fig2.savefig('figure-2/pr3/s%s' % i, dpi=200)
            plt.close(fig2)
        ax1.set_xlim(time1[:l].min(), time1[:l].max())
        #ax1.set_ylim(-4, 1.9)
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('figure-2/pr3', dpi=200)
        # do another one with zooms
        ax1.set_xlim(5000, 7000)
        #ax1.set_ylim(-2, 1.7)
        fig1.tight_layout()
        fig1.savefig('figure-2/pr3-z', dpi=200)
        #plt.show()
        plt.close(fig1)

        # Cache it
        torch.save(current1, 'figure-2/yc-pr3.pt')
        torch.save(pred_y_o, 'figure-2/yo-pr3.pt')
        torch.save(pred_y_1, 'figure-2/y1-pr3.pt')
        torch.save(pred_y_2, 'figure-2/y2-pr3.pt')
        ...
        for i in range(9):
            ...
            ax2.legend()
            fig2.tight_layout()
            fig2.savefig('figure-2/pr5/s%s' % i, dpi=200)
            plt.close(fig2)
        ax1.set_xlim(timep3[:l].min(), timep3[:l].max())
        #ax1.set_ylim(-3, 7.5)
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('figure-2/pr5', dpi=300)
        # do another one with zooms
        #ax1.set_xlim(1175, 1475)
        #ax1.set_ylim(-2.5, 7)
        #fig1.tight_layout()
        #fig1.savefig('figure-2/pr5-z', dpi=300)
        #plt.show()
        plt.close(fig1)

        # Cache it
        torch.save(currentp3, 'figure-2/yc-pr5.pt')
        torch.save(pred_y_o, 'figure-2/yo-pr5.pt')
        torch.save(pred_y_1, 'figure-2/y1-pr5.pt')
        torch.save(pred_y_2, 'figure-2/y2-pr5.pt')
...
plt.savefig('figure-2/fig2.pdf', format='pdf', pad_inches=0.02,
            bbox_inches='tight')
fig.canvas.start_event_loop(sys.float_info.min)  # Silence Tkinter callback
plt.savefig('figure-2/fig2', pad_inches=0.02, dpi=300, bbox_inches='tight')
#plt.show()
plt.close()
