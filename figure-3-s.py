...
func_1.load_state_dict(torch.load('s1/model-state-dict.pt'))
func_1.eval()

func_2 = ODEFunc1_6_2().to(device)
func_2.load_state_dict(torch.load('s2/model-state-dict.pt', map_location=torch.device('cpu')))
func_2.eval()
...
def sim_data(func, time, voltage, time_torch, gg, y0, noise=None):
    ...
    sim = gg * pred_y[:, 0, 0] * pred_y[:, 0, 1] * (func._v(time_torch).to(device) - e)
    if noise:
...
if args.cached:
    currentp1 = torch.load('figure-3-s/yc-aps.pt')
    pred_y_o_aps = torch.load('figure-3-s/yo-aps.pt')
    pred_y_1_aps = torch.load('figure-3-s/y1-aps.pt')
    pred_y_2_aps = torch.load('figure-3-s/y2-aps.pt')
    currentp2 = torch.load('figure-3-s/yc-sinewave.pt')
    pred_y_o_sin = torch.load('figure-3-s/yo-sinewave.pt')
    pred_y_1_sin = torch.load('figure-3-s/y1-sinewave.pt')
    pred_y_2_sin = torch.load('figure-3-s/y2-sinewave.pt')
    current2 = torch.load('figure-3-s/yc-pr4.pt')
    pred_y_o_pr4 = torch.load('figure-3-s/yo-pr4.pt')
    pred_y_1_pr4 = torch.load('figure-3-s/y1-pr4.pt')
    pred_y_2_pr4 = torch.load('figure-3-s/y2-pr4.pt')
else:
    with torch.no_grad():
        ###
        ...
        #
        # True model
        current2 = sim_data(func_o, time2, voltage2, time2_torch, g, true_y0, noise_sigma)
        # Trained Neural ODE
        makedirs('figure-3-s/pr4')
        ...
        for i in range(16):
            ...
            ax2.legend()
            fig2.tight_layout()
            fig2.savefig('figure-3-s/pr4/s%s' % i, dpi=200)
            plt.close(fig2)
        ax1.set_xlim(time2[:l].min(), time2[:l].max())
        #ax1.set_ylim(-3, 7.5)
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('figure-3-s/pr4', dpi=200)
        # do another one with zooms
        ax1.set_xlim(1175, 1475)
        #ax1.set_ylim(-2.5, 7)
        fig1.tight_layout()
        fig1.savefig('figure-3-s/pr4-z', dpi=200)
        #plt.show()
        plt.close(fig1)

        # Cache it
        torch.save(current2, 'figure-3-s/yc-pr4.pt')
        torch.save(pred_y_o, 'figure-3-s/yo-pr4.pt')
        torch.save(pred_y_1, 'figure-3-s/y1-pr4.pt')
        torch.save(pred_y_2, 'figure-3-s/y2-pr4.pt')
        ...
        #
        # True model
        currentp2 = sim_data(func_o, timep2, voltagep2, timep2_torch, g, true_y0, noise_sigma)
        # Trained Neural ODE
        ...
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('figure-3-s/sinewave', dpi=200)
        #plt.show()
        plt.close(fig1)

        # Cache it
        torch.save(currentp2, 'figure-3-s/yc-sinewave.pt')
        torch.save(pred_y_o, 'figure-3-s/yo-sinewave.pt')
        torch.save(pred_y_1, 'figure-3-s/y1-sinewave.pt')
        torch.save(pred_y_2, 'figure-3-s/y2-sinewave.pt')
        ...
        #
        # True model
        currentp1 = sim_data(func_o, timep1, voltagep1, timep1_torch, g, true_y0, noise_sigma)
        # Trained Neural ODE
        ...
        fig1.savefig('figure-3-s/aps', dpi=200)
        plt.close(fig1)

        # Cache it
        torch.save(currentp1, 'figure-3-s/yc-aps.pt')
        torch.save(pred_y_o, 'figure-3-s/yo-aps.pt')
        torch.save(pred_y_1, 'figure-3-s/y1-aps.pt')
        torch.save(pred_y_2, 'figure-3-s/y2-aps.pt')
...
plt.savefig('figure-3-s/fig3-s.pdf', format='pdf', pad_inches=0.02,
            bbox_inches='tight')
fig.canvas.start_event_loop(sys.float_info.min)  # Silence Tkinter callback
plt.savefig('figure-3-s/fig3-s', pad_inches=0.02, dpi=300, bbox_inches='tight')
#plt.show()
plt.close()
