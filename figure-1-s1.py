...
if args.cached:
    current1 = torch.load('figure-1-s1/yc-pr3.pt')
    currentp3 = torch.load('figure-1-s1/yc-pr4.pt')
    current1o = torch.load('figure-1-s1/yco-pr3.pt')
    currentp3o = torch.load('figure-1-s1/yco-pr4.pt')
else:
    with torch.no_grad():
        ###
        ### Training protocols
        ...
        torch.save(current1, 'figure-1-s1/yc-pr3.pt')
        torch.save(currentp3, 'figure-1-s1/yc-pr4.pt')

        current1o = sim_gt(func_o, time1, voltage1, time1_torch, true_y0)
        currentp3o = sim_gt(func_o, timep3, voltagep3, timep3_torch, true_y0)
        torch.save(current1, 'figure-1-s1/yco-pr3.pt')
        torch.save(currentp3, 'figure-1-s1/yco-pr4.pt')
...
for f in idxp3:
    ...
    ax1.plot(voltagep3[::ds2][i:f], currentp3o[0][::ds2].numpy()[i:f], dadt, alpha=0.8, color='#bcbddc', label='__nolegend__' if i!=0 else 'Pr4')
...
i = 0
for f in idxp3:
    ax2.plot(voltagep3[::ds2][i:f], currentp3[0][::ds2].numpy()[i:f], currentp3[1][::ds2].numpy()[i:f], alpha=0.8, color='#bcbddc', label='__nolegend__' if i!=0 else 'Pr4')
...
# Done
#
plt.savefig('figure-1-s1/fig1-s1.pdf', format='pdf', pad_inches=0.02,
            bbox_inches='tight')
fig.canvas.start_event_loop(sys.float_info.min)  # Silence Tkinter callback
plt.savefig('figure-1-s1/fig1-s1', pad_inches=0.02, dpi=300, bbox_inches='tight')
#plt.show()
plt.close()
