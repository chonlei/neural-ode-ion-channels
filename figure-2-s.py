...
parser = argparse.ArgumentParser('IKr NN ODE syn. data plot 1 for supplment.')
...
if args.cached:
    current1 = torch.load('figure-2-s/yc-pr3.pt')
    pred_y_o_pr3 = torch.load('figure-2-s/yo-pr3.pt')
    pred_y_1_pr3 = torch.load('figure-2-s/y1-pr3.pt')
    pred_y_2_pr3 = torch.load('figure-2-s/y2-pr3.pt')
    currentp3 = torch.load('figure-2-s/yc-pr5.pt')
    pred_y_o_pr5 = torch.load('figure-2-s/yo-pr5.pt')
    pred_y_1_pr5 = torch.load('figure-2-s/y1-pr5.pt')
    pred_y_2_pr5 = torch.load('figure-2-s/y2-pr5.pt')
else:
    with torch.no_grad():
        ###
        ### Training protocols
        ...
        #
        # True model
        current1 = sim_data(func_o, time1, voltage1, time1_torch, g, true_y0, noise_sigma)
        # Trained Neural ODE
        makedirs('figure-2-s/pr3')
        ...
        for i in range(7):
            ...
            ax2.legend()
            fig2.tight_layout()
            fig2.savefig('figure-2-s/pr3/s%s' % i, dpi=200)
            plt.close(fig2)
        ax1.set_xlim(time1[:l].min(), time1[:l].max())
        #ax1.set_ylim(-4, 1.9)
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('figure-2-s/pr3', dpi=200)
        # do another one with zooms
        ax1.set_xlim(5000, 7000)
        #ax1.set_ylim(-2, 1.7)
        fig1.tight_layout()
        fig1.savefig('figure-2-s/pr3-z', dpi=200)
        #plt.show()
        plt.close(fig1)

        # Cache it
        torch.save(current1, 'figure-2-s/yc-pr3.pt')
        torch.save(pred_y_o, 'figure-2-s/yo-pr3.pt')
        torch.save(pred_y_1, 'figure-2-s/y1-pr3.pt')
        torch.save(pred_y_2, 'figure-2-s/y2-pr3.pt')
        ...
        # Pr5
        #
        makedirs('figure-2-s/pr5')
        # True model
        currentp3 = sim_data(func_o, timep3, voltagep3, timep3_torch, g, true_y0, noise_sigma)
        # Trained Neural ODE
        ...
        for i in range(9):
            ...
            ax2.legend()
            fig2.tight_layout()
            fig2.savefig('figure-2-s/pr5/s%s' % i, dpi=200)
            plt.close(fig2)
        ax1.set_xlim(timep3[:l].min(), timep3[:l].max())
        #ax1.set_ylim(-3, 7.5)
        ax1.legend()
        fig1.tight_layout()
        fig1.savefig('figure-2-s/pr5', dpi=300)
        # do another one with zooms
        #ax1.set_xlim(1175, 1475)
        #ax1.set_ylim(-2.5, 7)
        #fig1.tight_layout()
        #fig1.savefig('figure-2-s/pr5-z', dpi=300)
        #plt.show()
        plt.close(fig1)

        # Cache it
        torch.save(currentp3, 'figure-2-s/yc-pr5.pt')
        torch.save(pred_y_o, 'figure-2-s/yo-pr5.pt')
        torch.save(pred_y_1, 'figure-2-s/y1-pr5.pt')
        torch.save(pred_y_2, 'figure-2-s/y2-pr5.pt')
...
plt.savefig('figure-2-s/fig2-s.pdf', format='pdf', pad_inches=0.02,
            bbox_inches='tight')
fig.canvas.start_event_loop(sys.float_info.min)  # Silence Tkinter callback
plt.savefig('figure-2-s/fig2-s', pad_inches=0.02, dpi=300, bbox_inches='tight')
#plt.show()
plt.close()
